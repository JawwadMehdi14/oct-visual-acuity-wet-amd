...
class ImageLevelDataset(Dataset):
    def __init__(self, json_path, image_root, transform=None):
        self.samples = []  # Each item = (image_path, label, patient_id)
...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        resnet.conv1 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)  # grayscale input
        self.feature_extractor = nn.Sequential(*list(resnet.children())[:-1])  # no FC
...
train_idx, test_idx = train_test_split(range(len(dataset)), test_size=0.2, stratify=labels, random_state=seed)
...
for epoch in range(num_epochs):
    ...
    for images, labels, _ in loop:
        images = images.squeeze(1).to(device)  # shape: [B, 1, 224, 224]
