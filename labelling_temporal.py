...
bscan_root = r"E:\Dataset_PNG"
output_json = "AMD_Label_Delta.json"
...
for idx, row in tqdm(df.iterrows(), total=len(df), desc="Processing patients"):
    ...
    # Store BCVA values temporarily for delta calculation
    bcva_values = {}
    ...
    for year in range(1, 14):
        ...
        bcva_values[year] = bcva_val  # Store current year BCVA
        ...
        if year == 1:
            ...
            # Find most recent available prior BCVA
            previous_year = None
            for y in reversed(range(1, year)):
                if y in bcva_values:
                    previous_year = y
                    break

            if previous_year is not None:
                delta = bcva_val - bcva_values[previous_year]
                label = "improved" if delta >= 0.1 else "not improved"
            else:
                continue  # Skip if no previous BCVA at all
